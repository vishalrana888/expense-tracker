    ...
    if (user) {
      bcrypt.compare(password, user.password, (hasherr, hashresponse) => {
        if (hasherr) {
          ...
          return res.status(500).json({ message: "Something went wrong in authentication", success: false });
        }
        if (hashresponse === true) {
          ...
          return res.status(401).json({ message: "User not authorized. Password Incorrect.", success: false });
        }
      });
    } else {
