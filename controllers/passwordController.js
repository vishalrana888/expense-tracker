        ...
        if (!user) {
            ...
            if (addForget) {
                ...
                await transporter.sendMail(msg, function (err, info) {
                    if (err) {
                        return console.log("Error occurred in sending mail..", err);
                    } else {
                ...
                return res.status(202).json({ message: 'Reset initiated email sent...Please Reset Password using link in mail. ', success: true })
            } else {
                    ...
                    bcrypt.genSalt(saltRounds, function (err, salt) {
                        ...
                        bcrypt.hash(newpassword, salt, function (err, hash) {
                            ...
                            user.update({ password: hash }).then(() => {
                                res.status(201).json({ message: 'Successfully updated the new password. Please login using the new Password.' })
                                return;
                            })
                        });
                    });
